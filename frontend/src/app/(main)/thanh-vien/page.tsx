...
export default function PeopleListPage() {
    ...
    const collapseAll = () => {
        const allGens = new Set(grouped.map(([gen]) => gen));
        setCollapsedGens(allGens);
    };
            ...
            try {
                ...
                if (!error && data) {
                    setPeople(data.map((row: Record<string, unknown>) => ({
                        ...
                        isPatrilineal: (row.is_patrilineal as boolean) ?? true,
    ...
    // Group by generation, sort within each group: chính tộc first, then by birthOrder, then name
    const grouped = useMemo(() => {
        ...
        for (const [, members] of map) {
            members.sort((a, b) => {
                // 1. Chính tộc (is_patrilineal = true) first
                if (a.isPatrilineal !== b.isPatrilineal) return a.isPatrilineal ? -1 : 1;
                // 2. By birth order (ascending, nulls last)
                const aOrder = a.birthOrder ?? 9999;
                const bOrder = b.birthOrder ?? 9999;
            ...
            <div className="flex items-start justify-between">
                ...
                {!loading && grouped.length > 1 && (
                    <div className="flex gap-1.5">
                        <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={expandAll}>Mở tất cả</Button>
                        <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={collapseAll}>Thu gọn</Button>
                    </div>
                )}
            </div>
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {grouped.map(([gen, members]) => {
                        ...
                        const patriCount = members.filter(m => m.isPatrilineal).length;
                        const ngoaiCount = members.length - patriCount;
                        return (
                            <Card key={gen} className="overflow-hidden">
                                <CardContent className="p-0">
                                    {/* Generation header — clickable to toggle */}
                                    <button
                                        className="w-full px-4 py-2.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex items-center justify-between hover:bg-slate-100/50 dark:hover:bg-slate-700/30 transition-colors cursor-pointer select-none"
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            {ngoaiCount > 0 && (
                                                <Badge variant="outline" className="text-[10px] text-muted-foreground">
                                                    {patriCount} chính tộc · {ngoaiCount} ngoại tộc
                                                </Badge>
                                            )}
                                        </div>
                                    </button>
